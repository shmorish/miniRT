...
/*   Updated: 2024/04/11 00:30:03 by tsishika         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_screen	screen_init(int x,
						int y,
						t_data *data)
{
	t_screen		screen;
	...
	screen.camera_to_screen = screen.w_s / \
		(2.0 * tan(data->camera.fov * M_PI / 360.0));
						...
						int y)
{
	...
	const t_vector	camera_dir = ray_obj_cross_pos(data->camera.coordinate, \
		screen.camera_to_screen, data->camera.direction);
	const t_vector	coordinate = ray_obj_cross_pos(\
		ray_obj_cross_pos(camera_dir, screen.f_x, screen.screen_x), \
		screen.f_y, screen.screen_y);
